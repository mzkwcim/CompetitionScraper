...
namespace CompetitionScraper
{
    ...
    {
        internal Dictionary<string, string> GetUrls(string clubUrl)
        {
            ...
            Parallel.ForEach(athletesCategories, athlete =>
            {
                for (int j = 0; j < 2; j++)
                {
                    for (int i = 0; i < 2; i++)
                    {
                        string changedLink = link.Replace("course=LCM", $"{poolLength[i]}").Replace("gender=1", $"{genderList[j]}").Replace("agegroup=0", $"{athlete}");
